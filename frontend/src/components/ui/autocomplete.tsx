/* وندورشده از shadcnspace — «Autocomplete 05 / Async Search».
   سازوکار دست‌نخورده است. سه چیز برای این پروژه عوض شده:
   ۱) کلاس‌های تِیل‌ویند ۴ به معادلِ تِیل‌ویند ۳ برگردانده شده‌اند
      (data-[…]، aria-[…]، ring-[3px]، outline-none، :not(:empty)).
   ۲) دکمه‌های clear/trigger از right-* به end-* رفته‌اند تا در صفحه‌ی
...
interface AutocompleteCtx {
  /** ریشه‌ی شناسه‌ها — بدون آن `aria-activedescendant` نمی‌شود ساخت و
      صفحه‌خوان هیچ‌وقت نمی‌گوید کاربر روی کدام گزینه ایستاده است. */
  baseId: string
  open: boolean
  ...
  itemsRef: React.RefObject<ItemMeta[]>
...
interface AutocompleteProps {
  ...
  /** کنترل‌شده از بیرون — لازم است تا چیزی مثل دکمه‌ی گفتار بتواند
      متن را داخلِ فیلد بنویسد. مثل `value` و `open`، اگر ندهی
      کامپوننت خودش نگه‌اش می‌دارد. */
  inputValue?: string
...
const Autocomplete = ({
  ...
  const inputValue = inputValueProp !== undefined ? inputValueProp : inputValueState
  ...
  const setInputValue = useCallback(
    (v: string) => {
      if (inputValueProp === undefined) setInputValueState(v)
...
const inputVariants = cva(
  'text-foreground placeholder:text-muted-foreground [[readonly]]:bg-muted/80 border-input focus-visible:border-ring aria-[invalid=true]:ring-destructive/20 dark:aria-[invalid=true]:ring-destructive/40 aria-[invalid=true]:border-destructive dark:aria-[invalid=true]:border-destructive/50 dark:bg-input/30 focus-visible:ring-ring/50 flex w-full rounded-lg border bg-transparent text-sm transition-colors outline-none focus-visible:ring-[3px] disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-[invalid=true]:ring-[3px] [[readonly]]:cursor-not-allowed',
  {
    variants: {
      size: {
        // قد و پدینگ را فراخوان می‌دهد. بدون این، px-2.5 این‌جا و
        // ps/pe فراخوان هر دو در خروجی می‌مانند و tailwind-merge هم
        // آن‌ها را هم‌گروه نمی‌بیند — یعنی پدینگِ فیلد به ترتیبِ تصادفیِ
        // CSS واگذار می‌شود.
        none: '',
        sm: 'h-7 px-2 [&~[data-slot=autocomplete-clear]]:end-1.5 [&~[data-slot=autocomplete-trigger]]:end-1.5',
        default:
          'h-8 px-2.5 [&~[data-slot=autocomplete-clear]]:end-[0.4375rem] [&~[data-slot=autocomplete-trigger]]:end-[0.4375rem]',
...
const AutocompleteInput = ({
  ...
  const { baseId, open, setOpen, inputValue, setInputValue, inputWrapperRef, inputRef, disabled, highlightedValue, highlightNext, highlightPrev, selectHighlighted } =
    useAc()
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    switch (e.key) {
      ...
      case 'ArrowUp':
        ...
        break
      case 'Enter':
        /* فقط وقتی رویداد را می‌بلعیم که واقعاً گزینه‌ای را انتخاب کرده
           باشیم. وگرنه Enter باید به فرمِ بالادست برسد — کاربری که
           لیست را ندید و فقط متن نوشت، انتظار جستجو دارد. */
  ...
  return (
    <div ref={inputWrapperRef} className='relative w-full'>
      <input
        ...
        aria-activedescendant={open && highlightedValue ? `${baseId}-${highlightedValue}` : undefined}
        disabled={disabled}
        value={inputValue}
        onChange={e => {
...
const AutocompleteList = ({ className, id, ...props }: AutocompleteListProps) => {
  const { baseId } = useAc()
  return (
    <div
...
const AutocompleteItem = ({
  ...
  const { baseId, selectedValue, highlightedValue, setHighlightedValue, selectItem, registerItem, unregisterItem } =
    useAc()
